<!--babbage.cs.missouri.edu/~njmnbb/cs3380/lab3/lab3.php-->
<!DOCTYPE html>
<html>
	<head>
		<meta charset=UTF-8>
...
<body>
	...
	if (isset($_POST['submit'])){
		...
		switch ($_POST['query']){
			...
			//For each country display the capital city name and the percentage of the population that lives in the capital for each country.
			//Sort the results from largest percentage to smallest percentage
			case 10:
				$query = 	'SELECT city.name AS capital_name, country.name AS country_name, ((city.population * 100) / country.population) AS urban_pct 
							FROM lab3.city 
							INNER JOIN lab3.country USING(country_code) 
			...
			//Find all official languages, the country for which it is spoken, and the percentage of speakers (percentage of speakers is calculated as percentage spoken times country population divided by 100).
			//Order results number of speakers with the most popular language first.
			case 11:
				$query = 	'SELECT country.name, country_language.language, ROUND((country_language.percentage * country.population) / 100) AS speakers 
							FROM lab3.country
							INNER JOIN lab3.country_language USING(country_code)
			...
			//Find the name, region, GNP, old GNP, and real change in GNP for the countries who have most improved their relative wealth.
			//The real change in GNP is defined as (gnp - gnp old)/gnp old.
			//Order results by real change with the most improved country first.
			//Also, this data is missing some entries for gnp and gnp old.
			//Filter these missing entries out by only returning countries where gnp IS NOT NULL and gnp old IS NOT NULL.
			case 12:
				 $query = 	'SELECT name, region, gnp, gnp_old, ((gnp - gnp_old) / gnp_old) AS real_change 
                    		FROM lab3.country 
